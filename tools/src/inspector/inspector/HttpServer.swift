...
// See LICENSE file for details.
  ...
  func shareFilesFromDirectory(
    ...
    return { r in
      ...
      if let file = try? (directoryPath + String.pathSeparator + fileRelativePath.value)
        .openForReading()
      {
        return .raw(
          200, "OK", [:],
    ...
    private func extractQueryParams(_ url: String) -> [(String, String)] {
      ...
      return query.components(separatedBy: "&")
        .reduce([(String, String)]()) { (c, s) -> [(String, String)] in
          ...
          guard
            let name = String(
              s[s.startIndex..<nameEndIndex]).removingPercentEncoding
          else {
            return c
          }
          ...
          guard
            let value = String(
              s[valueStartIndex..<s.endIndex]).removingPercentEncoding
          else {
            return c + [(name, "")]
          }
          return c + [(name, value)]
  ...
  class HttpRequest {
    ...
    func hasTokenForHeader(_ headerName: String, token: String) -> Bool {
      ...
      return
        headerValue.components(separatedBy: ",").filter({
    ...
    private func nextMultiPart(
      _ generator: inout IndexingIterator<[UInt8]>,
      boundary: String,
      isFirst: Bool
    ) -> MultiPart? {
      if isFirst {
    ...
    private func nextMultiPartBody(
      _ generator: inout IndexingIterator<[UInt8]>,
      boundary: String
    ) -> [UInt8]? {
      var body = [UInt8]()
  ...
  class HttpRouter {
    ...
    func route(_ method: String?, path: String) -> (
  ...
  class HttpServerIO {
    ...
    var operating: Bool { return self.state == .running }
  ...
  private func evaluate(_ node: String, _ attrs: [String: String?] = [:], _ c: Closure) {
    ...
    private func sendfileImpl(
      _ source: UnsafeMutablePointer<FILE>,
      _ view: Int32,
      _: off_t,
      ...
      if forceIPv4 {
        ...
        if let address = listenAddress {
          if address.withCString({
            cstring in inet_pton(AF_INET, cstring, &addr.sin_addr)
          }) == 1 {
        ...
        if let address = listenAddress {
          if address.withCString({
            cstring in inet_pton(AF_INET6, cstring, &addr.sin6_addr)
          }) == 1 {
  ...
  extension String {
    ...
    private static let CODES = [UInt8](
  ...
  class WebSocketSession: Hashable, Equatable {
    ...
    func writeBinary(_ binary: [UInt8]) {
      self.writeBinary(ArraySlice(binary))
    }

    func writeBinary(_ binary: ArraySlice<UInt8>) {
