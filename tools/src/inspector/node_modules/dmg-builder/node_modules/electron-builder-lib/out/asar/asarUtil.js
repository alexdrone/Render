  ...
  createPackageFromFiles(fileSet, packager) {
    ...
    return (0, _bluebirdLst().coroutine)(function* () {
      ...
      if (_this2.options.smartUnpack !== false) {
        yield (0, _unpackDetector().detectUnpackedDirs)(fileSet, unpackedDirs, unpackedDest, rootForAppFilesWithoutAsar);
      }
        ...
        var _ref = (0, _bluebirdLst().coroutine)(function* (filePathInArchive, dirNode) {
          for (const dir of unpackedDirs) {
            if (filePathInArchive.length > dir.length + 2 && filePathInArchive[dir.length] === path.sep && filePathInArchive.startsWith(dir)) {
              ...
              yield (0, _fsExtraP().ensureDir)(path.join(unpackedDest, filePathInArchive));
              break;
            }
          }
        });
      ...
      for (let i = 0, n = fileSet.files.length; i < n; i++) {
        ...
        if (isUnpacked) {
          if (!dirNode.unpacked && !dirToCreateForUnpackedFiles.has(fileParent)) {
            ...
            yield (0, _fsExtraP().ensureDir)(path.join(unpackedDest, fileParent));
          }

          const unpackedFile = path.join(unpackedDest, pathInArchive);
          taskManager.addTask((0, _appFileCopier().copyFileOrData)(fileCopier, newData, file, unpackedFile, stat));
