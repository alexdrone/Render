...
extension UIImage {
  ...
  internal class func delayForImageAtIndex(_ index: Int, source: CGImageSource!) -> Double {
    ...
    let ptr = UnsafeMutablePointer<UnsafeRawPointer?>.allocate(capacity: 0)
    if CFDictionaryGetValueIfPresent(
      cfProperties,
      Unmanaged.passUnretained(kCGImagePropertyGIFDictionary).toOpaque(), ptr) == false {
      return delay
    }
    let gifProperties:CFDictionary = unsafeBitCast(ptr.pointee, to: CFDictionary.self)
    // Get delay time.
