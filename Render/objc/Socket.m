...
#ifndef AsyncSocketLoggingEnabled
#define AsyncSocketLoggingEnabled 0
#endif

#if AsyncSocketLoggingEnabled

// Logging Enabled - See log level below

// Logging uses the CocoaLumberjack framework (which is also  based).
...
#define LogAsync   YES
#define LogContext AsyncSocketLoggingContext
...
#ifndef AsyncSocketLogLevel
#define AsyncSocketLogLevel LOG_LEVEL_VERBOSE
#endif

// Log levels : off, error, warn, info, verbose
static const int logLevel = AsyncSocketLogLevel;
...
NSString *const AsyncSocketQueueName = @"AsyncSocket";
NSString *const AsyncSocketThreadName = @"AsyncSocket-CFStream";

NSString *const AsyncSocketManuallyEvaluateTrust = @"AsyncSocketManuallyEvaluateTrust";
#if TARGET_OS_IPHONE
NSString *const AsyncSocketUseCFStreamForTLS = @"AsyncSocketUseCFStreamForTLS";
#endif
NSString *const AsyncSocketSSLPeerID = @"AsyncSocketSSLPeerID";
NSString *const AsyncSocketSSLProtocolVersionMin = @"AsyncSocketSSLProtocolVersionMin";
NSString *const AsyncSocketSSLProtocolVersionMax = @"AsyncSocketSSLProtocolVersionMax";
NSString *const AsyncSocketSSLSessionOptionFalseStart = @"AsyncSocketSSLSessionOptionFalseStart";
NSString *const AsyncSocketSSLSessionOptionSendOneByteRecord = @"AsyncSocketSSLSessionOptionSendOneByteRecord";
NSString *const AsyncSocketSSLCipherSuites = @"AsyncSocketSSLCipherSuites";
#if !TARGET_OS_IPHONE
NSString *const AsyncSocketSSLDiffieHellmanParameters = @"AsyncSocketSSLDiffieHellmanParameters";
#endif
...
@interface AsyncSocketPreBuffer : NSObject
{
	uint8_t *preBuffer;
...
@implementation AsyncSocketPreBuffer
...
- (NSUInteger)readLengthForTermWithPreBuffer:(AsyncSocketPreBuffer *)preBuffer found:(BOOL *)foundPtr;
...
@implementation AsyncSocket
{
	...
	__weak id<AsyncSocketDelegate> delegate;
	...
	AsyncSocketPreBuffer *preBuffer;
	...
	AsyncSocketPreBuffer *sslPreBuffer;
...
- (id)initWithDelegate:(id<AsyncSocketDelegate>)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq
{
	if((self = [super init]))
	{
		delegate = aDelegate;
		...
		{
			socketQueue = dispatch_queue_create([AsyncSocketQueueName UTF8String], NULL);
		}
		...
		preBuffer = [[AsyncSocketPreBuffer alloc] initWithCapacity:(1024 * 4)];
...
+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<AsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq error:(NSError**)error {
  return [self socketFromConnectedSocketFD:socketFD delegate:aDelegate delegateQueue:dq socketQueue:NULL error:error];
}

+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<AsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq error:(NSError* __autoreleasing *)error
{
  __block BOOL errorOccured = NO;
  
  AsyncSocket *socket = [[[self class] alloc] initWithDelegate:aDelegate delegateQueue:dq socketQueue:sq];
  ...
  dispatch_sync(socket->socketQueue, ^{ @autoreleasepool {
    ...
    if (retVal)
    {
      NSString *errMsg = NSLocalizedStringWithDefaultValue(@"AsyncSocketOtherError",
      ...
      errorOccured = YES;
      if (error)
        *error = [NSError errorWithDomain:AsyncSocketErrorDomain code:AsyncSocketOtherError userInfo:userInfo];
      return;
    }
...
- (void)getDelegate:(id<AsyncSocketDelegate> *)delegatePtr delegateQueue:(dispatch_queue_t *)delegateQueuePtr
{
	if (dispatch_get_specific(IsOnSocketQueueOrTargetQueueKey))
	{
...
- (BOOL)acceptOnInterface:(NSString *)inInterface port:(uint16_t)port error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		if (enableIPv4)
		{
			...
			__weak AsyncSocket *weakSelf = self;
			
			dispatch_source_set_event_handler(accept4Source, ^{ @autoreleasepool {
			#pragma clang diagnostic push
			#pragma clang diagnostic warning "-Wimplicit-retain-self"
				
				__strong AsyncSocket *strongSelf = weakSelf;
...
- (BOOL)doAccept:(int)parentSocketFD
{
	...
	if (delegateQueue)
	{
		...
		dispatch_async(delegateQueue, ^{ @autoreleasepool {
			...
			// Create AsyncSocket instance for accepted socket
			
			AsyncSocket *acceptedSocket = [[[self class] alloc] initWithDelegate:theDelegate
...
- (NSError *)badConfigError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:AsyncSocketErrorDomain code:AsyncSocketBadConfigError userInfo:userInfo];
}
...
- (NSError *)badParamError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:AsyncSocketErrorDomain code:AsyncSocketBadParamError userInfo:userInfo];
}
...
- (NSError *)connectTimeoutError
{
	...
	return [NSError errorWithDomain:AsyncSocketErrorDomain code:AsyncSocketConnectTimeoutError userInfo:userInfo];
}
...
- (NSError *)readTimeoutError
{
	...
	return [NSError errorWithDomain:AsyncSocketErrorDomain code:AsyncSocketReadTimeoutError userInfo:userInfo];
}
...
- (NSError *)writeTimeoutError
{
	...
	return [NSError errorWithDomain:AsyncSocketErrorDomain code:AsyncSocketWriteTimeoutError userInfo:userInfo];
}

- (NSError *)connectionClosedError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"AsyncSocketClosedError",
	                                                     @"AsyncSocket", [NSBundle mainBundle],
	                                                     @"Socket closed by remote peer", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:AsyncSocketErrorDomain code:AsyncSocketClosedError userInfo:userInfo];
}
...
- (NSError *)otherError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:AsyncSocketErrorDomain code:AsyncSocketOtherError userInfo:userInfo];
}
...
- (BOOL)usingCFStreamForTLS
{
	...
	{
		// The startTLS method was given the AsyncSocketUseCFStreamForTLS flag.
...
- (float)progressOfReadReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr
{
	...
	dispatch_block_t block = ^{
		...
		if (!currentRead || ![currentRead isKindOfClass:[AsyncReadPacket class]])
		{
...
- (void)maybeDequeueRead
{
	...
	if ((currentRead == nil) && (flags & kConnected))
	{
		if ([readQueue count] > 0)
		{
			...
			{
				LogVerbose(@"Dequeued AsyncSpecialPacket");
			...
			{
				LogVerbose(@"Dequeued AsyncReadPacket");
...
- (void)doReadData
{
	...
	if ([self usingCFStreamForTLS])
	{
		...
		// Requested CFStream, rather than SecureTransport, for TLS (via AsyncSocketUseCFStreamForTLS)
...
- (void)completeCurrentRead
{
	...
	{
		AsyncReadPacket *theRead = currentRead; // Ensure currentRead retained since result may not own buffer
...
- (float)progressOfWriteReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr
{
	...
	dispatch_block_t block = ^{
		...
		if (!currentWrite || ![currentWrite isKindOfClass:[AsyncWritePacket class]])
		{
...
- (void)maybeDequeueWrite
{
	...
	if ((currentWrite == nil) && (flags & kConnected))
	{
		if ([writeQueue count] > 0)
		{
			...
			{
				LogVerbose(@"Dequeued AsyncWritePacket");
...
- (void)doWriteData
{
	...
	// Note: This method is not called if currentWrite is a AsyncSpecialPacket (startTLS packet)
...
- (void)maybeStartTLS
{
	...
	if ((flags & kStartingReadTLS) && (flags & kStartingWriteTLS))
	{
		...
		#if TARGET_OS_IPHONE
		{
			...
			NSNumber *value = [tlsSettings objectForKey:AsyncSocketUseCFStreamForTLS];
...
{
	AsyncSocket *asyncSocket = (__bridge AsyncSocket *)connection;
...
- (void)ssl_startTLS
{
	...
	BOOL shouldManuallyEvaluateTrust = [[tlsSettings objectForKey:AsyncSocketManuallyEvaluateTrust] boolValue];
	...
	//  3. AsyncSocketSSLPeerID
	//  4. AsyncSocketSSLProtocolVersionMin
	//  5. AsyncSocketSSLProtocolVersionMax
	//  6. AsyncSocketSSLSessionOptionFalseStart
	//  7. AsyncSocketSSLSessionOptionSendOneByteRecord
	//  8. AsyncSocketSSLCipherSuites
	//  9. AsyncSocketSSLDiffieHellmanParameters (Mac)
	//
	...
	value = [tlsSettings objectForKey:AsyncSocketSSLPeerID];
	...
	{
		NSAssert(NO, @"Invalid value for AsyncSocketSSLPeerID. Value must be of type NSData."
		             @" (You can convert strings to data using a method like"
		             @" [string dataUsingEncoding:NSUTF8StringEncoding])");
		
		[self closeWithError:[self otherError:@"Invalid value for AsyncSocketSSLPeerID."]];
		return;
	}
	
	// 4. AsyncSocketSSLProtocolVersionMin
	
	value = [tlsSettings objectForKey:AsyncSocketSSLProtocolVersionMin];
	...
	{
		NSAssert(NO, @"Invalid value for AsyncSocketSSLProtocolVersionMin. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for AsyncSocketSSLProtocolVersionMin."]];
		return;
	}
	
	// 5. AsyncSocketSSLProtocolVersionMax
	
	value = [tlsSettings objectForKey:AsyncSocketSSLProtocolVersionMax];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for AsyncSocketSSLProtocolVersionMax. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for AsyncSocketSSLProtocolVersionMax."]];
		return;
	}
	
	// 6. AsyncSocketSSLSessionOptionFalseStart
	
	value = [tlsSettings objectForKey:AsyncSocketSSLSessionOptionFalseStart];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for AsyncSocketSSLSessionOptionFalseStart. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for AsyncSocketSSLSessionOptionFalseStart."]];
		return;
	}
	
	// 7. AsyncSocketSSLSessionOptionSendOneByteRecord
	
	value = [tlsSettings objectForKey:AsyncSocketSSLSessionOptionSendOneByteRecord];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for AsyncSocketSSLSessionOptionSendOneByteRecord."
		             @" Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for AsyncSocketSSLSessionOptionSendOneByteRecord."]];
		return;
	}
	
	// 8. AsyncSocketSSLCipherSuites
	
	value = [tlsSettings objectForKey:AsyncSocketSSLCipherSuites];
	if ([value isKindOfClass:[NSArray class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for AsyncSocketSSLCipherSuites. Value must be of type NSArray.");
		
		[self closeWithError:[self otherError:@"Invalid value for AsyncSocketSSLCipherSuites."]];
		return;
	}
	
	// 9. AsyncSocketSSLDiffieHellmanParameters
	
	#if !TARGET_OS_IPHONE
	value = [tlsSettings objectForKey:AsyncSocketSSLDiffieHellmanParameters];
	if ([value isKindOfClass:[NSData class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for AsyncSocketSSLDiffieHellmanParameters. Value must be of type NSData.");
		
		[self closeWithError:[self otherError:@"Invalid value for AsyncSocketSSLDiffieHellmanParameters."]];
		return;
	}
	#endif
	...
	if (value)
	{
		NSAssert(NO, @"Security option unavailable - kCFStreamSSLLevel"
		             @" - You must use AsyncSocketSSLProtocolVersionMin & AsyncSocketSSLProtocolVersionMax");
	...
	sslPreBuffer = [[AsyncSocketPreBuffer alloc] initWithCapacity:(1024 * 4)];
...
- (void)ssl_continueSSLHandshake
{
	...
	else if (status == errSSLPeerAuthCompleted)
	{
		...
		else
		{
			...
			NSString *msg = @"AsyncSocketManuallyEvaluateTrust specified in tlsSettings,"
...
- (void)cf_startTLS
{
	...
	NSAssert([currentRead isKindOfClass:[AsyncSpecialPacket class]], @"Invalid read packet for startTLS");
	NSAssert([currentWrite isKindOfClass:[AsyncSpecialPacket class]], @"Invalid write packet for startTLS");
...
+ (void)startCFStreamThreadIfNeeded
{
	...
	dispatch_once(&predicate, ^{
		...
		cfstreamThreadSetupQueue = dispatch_queue_create("AsyncSocket-CFStreamThreadSetup", DISPATCH_QUEUE_SERIAL);
	});
...
{
	[[NSThread currentThread] setName:AsyncSocketThreadName];
...
+ (void)scheduleCFStreams:(AsyncSocket *)asyncSocket
{
	LogTrace();
...
+ (void)unscheduleCFStreams:(AsyncSocket *)asyncSocket
{
	LogTrace();
	NSAssert([NSThread currentThread] == cfstreamThread, @"Invoked on wrong thread");
...
{
	AsyncSocket *asyncSocket = (__bridge AsyncSocket *)pInfo;
