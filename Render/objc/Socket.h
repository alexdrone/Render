...
extern NSString *const AsyncSocketQueueName;
extern NSString *const AsyncSocketThreadName;

extern NSString *const AsyncSocketManuallyEvaluateTrust;
#if TARGET_OS_IPHONE
extern NSString *const AsyncSocketUseCFStreamForTLS;
#endif
#define AsyncSocketSSLPeerName     (NSString *)kCFStreamSSLPeerName
#define AsyncSocketSSLCertificates (NSString *)kCFStreamSSLCertificates
#define AsyncSocketSSLIsServer     (NSString *)kCFStreamSSLIsServer
extern NSString *const AsyncSocketSSLPeerID;
extern NSString *const AsyncSocketSSLProtocolVersionMin;
extern NSString *const AsyncSocketSSLProtocolVersionMax;
extern NSString *const AsyncSocketSSLSessionOptionFalseStart;
extern NSString *const AsyncSocketSSLSessionOptionSendOneByteRecord;
extern NSString *const AsyncSocketSSLCipherSuites;
#if !TARGET_OS_IPHONE
extern NSString *const AsyncSocketSSLDiffieHellmanParameters;
#endif

#define AsyncSocketLoggingContext 65535


typedef NS_ENUM(NSInteger, AsyncSocketError) {
	AsyncSocketNoError = 0,           // Never used
	AsyncSocketBadConfigError,        // Invalid configuration
	AsyncSocketBadParamError,         // Invalid parameter was passed
	AsyncSocketConnectTimeoutError,   // A connect operation timed out
	AsyncSocketReadTimeoutError,      // A read operation timed out
	AsyncSocketWriteTimeoutError,     // A write operation timed out
	AsyncSocketReadMaxedOutError,     // Reached set maxLength without completing
	AsyncSocketClosedError,           // The remote peer closed the connection
	AsyncSocketOtherError,            // Description provided in userInfo
};
...
/**
 * AsyncSocket uses the standard delegate paradigm,
 ...
 * If you pass NULL, AsyncSocket will automatically create it's own socket queue.
...
- (instancetype)initWithDelegate:(nullable id<AsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq;
- (instancetype)initWithDelegate:(nullable id<AsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq;

/**
 * Create AsyncSocket from already connect BSD socket file descriptor
**/
...
+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<AsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq error:(NSError **)error;

#pragma mark Configuration

@property (atomic, weak, readwrite, nullable) id<AsyncSocketDelegate> delegate;
...
- (void)getDelegate:(id<AsyncSocketDelegate> __nullable * __nullable)delegatePtr delegateQueue:(dispatch_queue_t __nullable * __nullable)delegateQueuePtr;
- (void)setDelegate:(nullable id<AsyncSocketDelegate>)delegate delegateQueue:(nullable dispatch_queue_t)delegateQueue;
...
**/
- (void)synchronouslySetDelegate:(nullable id<AsyncSocketDelegate>)delegate;
- (void)synchronouslySetDelegateQueue:(nullable dispatch_queue_t)delegateQueue;
- (void)synchronouslySetDelegate:(nullable id<AsyncSocketDelegate>)delegate delegateQueue:(nullable dispatch_queue_t)delegateQueue;

/**
 * By default, both IPv4 and IPv6 are enabled.
 * 
 * For accepting incoming connections, this means AsyncSocket automatically supports both protocols,
 * and can simulataneously accept incoming connections on either protocol.
 * 
 * For outgoing connections, this means AsyncSocket can connect to remote hosts running either protocol.
 * If a DNS lookup returns only IPv4 results, AsyncSocket will automatically use IPv4.
 * If a DNS lookup returns only IPv6 results, AsyncSocket will automatically use IPv6.
 ...
 * When a connection is accepted, a new instance of AsyncSocket will be spawned to handle it,
 ...
 * 
 * Please note the recommended way of releasing a AsyncSocket instance (e.g. in a dealloc method)
 ...
 * This is due to the fact that AsyncSocket does NOT copy the data. It simply retains it.
 ...
 * 
 * - AsyncSocketManuallyEvaluateTrust
 ...
 *     So AsyncSocket will invoke the delegate method socket:shouldTrustPeer: passing the SecTrustRef.
 *
 ...
 *
 * - AsyncSocketUseCFStreamForTLS (iOS only)
 *     The value must be of type NSNumber, encapsulating a BOOL value.
 *     By default AsyncSocket will use the SecureTransport layer to perform encryption.
 ...
 *     
 *     However, if you absolutely must, you can instruct AsyncSocket to use the old-fashioned encryption
 *     technique by going through the CFStream instead. So instead of using SecureTransport, AsyncSocket
 ...
 *     
 *     Thus all the other keys in the given dictionary will be ignored by AsyncSocket,
 ...
 *
 * - AsyncSocketSSLProtocolVersionMin
 * - AsyncSocketSSLProtocolVersionMax
 ...
 *
 * - AsyncSocketSSLDiffieHellmanParameters (Mac OS X only)
 *     The value must be of type NSData.
 ...
 *     You MUST use manual trust evaluation instead (see AsyncSocketManuallyEvaluateTrust).
 ...
 *     You MUST use AsyncSocketSSLProtocolVersionMin & AsyncSocketSSLProtocolVersionMin instead.
...
/**
 * AsyncSocket maintains thread safety by using an internal serial dispatch_queue.
 ...
 * This is because the  API offers no mechanism to discover a queue's targetQueue.
 ...
 * - (dispatch_queue_t)newSocketQueueForConnectionFromAddress:(NSData *)address onSocket:(AsyncSocket *)sock
 * {
 ...
 *   The faster and more powerful  API isn't notified properly in this case.
 * 
...
**/
- (nullable dispatch_queue_t)newSocketQueueForConnectionFromAddress:(NSData *)address onSocket:(AsyncSocket *)sock;
 ...
 * The host parameter will be an IP address, not a DNS name.
 **/
- (void)socket:(AsyncSocket *)sock didConnectToUrl:(NSURL *)url;
...
**/
- (NSTimeInterval)socket:(AsyncSocket *)sock shouldTimeoutWriteWithTag:(long)tag
                                                                  elapsed:(NSTimeInterval)elapsed
                                                                bytesDone:(NSUInteger)length;
 ...
 * 
 * Note: If the AsyncSocket instance is deallocated while it is still connected,
...
**/
- (void)socketDidDisconnect:(AsyncSocket *)sock withError:(nullable NSError *)err;
 ...
 * - AsyncSocketManuallyEvaluateTrust == YES
 *
...
**/
- (void)socket:(AsyncSocket *)sock didReceiveTrust:(SecTrustRef)trust
